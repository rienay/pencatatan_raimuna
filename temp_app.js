...
async function pushPendingDeletes() {
  ...
  for (const item of pendingDeletes) {
    try {
      ...
      // Berhasil ΓåÆ hapus dari tombstone
  ...
  if (latestTxns.length === 0) {
    ...
    latestTxns.forEach(tx => {
      ...
      const syncBadge = tx.sync 
        ? '<span class="badge-sync synced" title="Tersinkronisasi">Γÿü∩╕Å</span>' 
        : '<span class="badge-sync unsynced" title="Belum disinkronkan">ΓÅ│</span>';
      ...
      tr.innerHTML = `
        ...
        <td class="no-print text-center" style="white-space: nowrap;">
          <button class="btn-icon btn-edit-action" title="Edit Transaksi" style="margin-right: 4px;">Γ£Å∩╕Å</button>
          <button class="btn-icon btn-view-action" title="Lihat Detail">≡ƒöì</button>
        </td>
      `;
    ...
    header.addEventListener('click', () => {
      ...
      headers.forEach(h => {
        ...
        if (h.getAttribute('data-sort') === state.sort.column) {
          icon.textContent = state.sort.direction === 'asc' ? 'Γû▓' : 'Γû╝';
        } else {
          icon.textContent = '';
        }
      });
...
function renderKategoriPage() {
  ...
  state.categories.forEach(cat => {
    ...
    if (cat.isDefault) {
      ...
      actionHtml = `<button class="btn-icon text-danger btn-delete-cat" data-id="${cat.id}">≡ƒùæ∩╕Å</button>`;
    }
  ...
  state.sources.forEach(src => {
    ...
    if (src.isDefault) {
      ...
      actionHtml = `<button class="btn-icon text-danger btn-delete-src" data-id="${src.id}">≡ƒùæ∩╕Å</button>`;
    }
...
function showToast(title, message, type = 'success') {
  ...
  toast.innerHTML = `
    <div class="toast-icon">${icons[type] || '≡ƒöö'}</div>
...
function exportUtangRecap(format) {
  ...
  if (format === 'CSV') {
    ...
    filtered.forEach((u, i) => {
      const tipeText = u.tipe === 'IN' ? 'Piutang (Masuk)' : 'Utang (Keluar)';
      const statusText = u.status === 'LUNAS' ? '<b style="color: #16a34a;">LUNAS Γ£à</b>' : '<b style="color: #dc2626;">BELUM LUNAS ΓÅ│</b>';
...
function renderUtangPage() {
  ...
  filtered.forEach(u => {
    ...
    const statusBadge = isPaid 
      ? '<span class="badge-status success">LUNAS Γ£à</span>' 
      : '<span class="badge-status warning">BELUM LUNAS ΓÅ│</span>';
    ...
    tr.innerHTML = `
      ...
      <td>${statusBadge}</td>
      <td class="text-center">
        <button class="btn-icon btn-delete-utang" data-id="${u.id}" title="Hapus Catatan Utang">≡ƒùæ∩╕Å</button>
      </td>
    `;
...
function renderDashboardQuickUtang() {
  ...
  if (pendingUtang.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted">Tidak ada utang yang belum lunas. ≡ƒÄë</td></tr>';
    return;
  }
