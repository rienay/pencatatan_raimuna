...
function parseKwitansiRow(row) {
  if (!row) return null;
  let no = row.no || row['No. Kwitansi'] || row['No.'] || row['No'] || row.noKwitansi || '';
  if (typeof no === 'string' && (no.startsWith('KW-') || (no.length >= 10 && /^\d+$/.test(no)))) {
    no = '';
  }
...
function setupNavigation() {
  ...
  links.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      } else if (targetSectionId === 'pemasukan-section') {
        populateDropdowns();
        renderPemasukanHistory();
      } else if (targetSectionId === 'pengeluaran-section') {
        populateDropdowns();
        renderPengeluaranHistory();
      } else if (targetSectionId === 'pengambilan-section') {
        populateDropdowns();
        renderPengambilanHistory();
      }
    });
  });
}
...
function renderApp() {
  ...
  if (activeSec) {
    ...
    else if (id === 'pemasukan-section') renderPemasukanHistory();
    else if (id === 'pengeluaran-section') renderPengeluaranHistory();
    else if (id === 'pengambilan-section') renderPengambilanHistory();
  }
}
...
function updateKwitansiNumberState() {
  const kwitansiNo = document.getElementById('kwitansi-no');
  const kwitansiTipeJenis = document.getElementById('kwitansi-tipe-jenis');
  const reqKwitansiNo = document.getElementById('req-kwitansi-no');
  const isTenant = kwitansiTipeJenis && (kwitansiTipeJenis.value === 'STAND' || kwitansiTipeJenis.value === 'TENANT');

  if (kwitansiNo) {
    if (isTenant) {
      kwitansiNo.value = '';
      kwitansiNo.placeholder = 'Dikosongkan (Tulis manual)';
      kwitansiNo.readOnly = true;
      kwitansiNo.style.backgroundColor = '#f1f5f9';
      kwitansiNo.style.cursor = 'not-allowed';
      if (reqKwitansiNo) reqKwitansiNo.style.display = 'none';
    } else {
      kwitansiNo.readOnly = false;
      kwitansiNo.style.backgroundColor = '';
      kwitansiNo.style.cursor = '';
      kwitansiNo.placeholder = 'Contoh: 001';
      if (reqKwitansiNo) reqKwitansiNo.style.display = 'inline';
      if (!kwitansiNo.value || kwitansiNo.value === '001' || kwitansiNo.value.length > 6) {
...
function getNextKwitansiNumber() {
  ...
  state.sponsorshipHistory.forEach(item => {
    const rawStr = String(item.no || '').trim();
    if (rawStr && /^\d+$/.test(rawStr) && rawStr.length <= 6) {
      const rawNo = parseInt(rawStr, 10);
...
function renderSponsorshipSection() {
  ...
  // Update receipt number state based on type (empty/disabled for tenant, auto-increment for sponsorship)
  updateKwitansiNumberState();
...
function renderSingleKwitansiHistoryTable(typeKey, items, tbodyId, countBadgeId, amountBadgeId, unitType) {
  ...
  items.forEach(item => {
    ...
    html += `
      <tr>
        <td><span class="kwitansi-no-badge">${item.no ? `No. ${item.no}` : 'No. (Manual)'}</span> ${badgeTag}</td>
        <td>${item.tgl || '-'}</td>
...
async function loadKwitansiFromHistory(id, autoPrint = true) {
  ...
  updateKwitansiNumberState();
  if (kwitansiNo && !isTenant && item.no) {
    kwitansiNo.value = item.no;
  }
  ...
  if (autoPrint) {
    ...
    showToast('Kwitansi Dimuat', `Data Kwitansi ${item.no ? `No. ${item.no}` : 'Tenant'} berhasil dimuat ke form.`, 'info');
...
function generateKwitansiHTML() {
  ...
  const isTenant = (tipeJenis === 'STAND' || tipeJenis === 'TENANT');
  const noInput = document.getElementById('kwitansi-no')?.value?.trim() || '';
  ...
  const guna = document.getElementById('kwitansi-guna')?.value || (tipeJenis === 'JELANTAH' ? 'PERSYARATAN MINYAK JELANTAH' : (isTenant ? 'PEMBAYARAN SEWA TENANT BOOTH RAIMUNA CABANG CILACAP TAHUN 2026' : 'SPONSORSHIP KEGIATAN RAIMUNA CABANG CILACAP TAHUN 2026'));
  ...
  // Nomor kwitansi: untuk tenant SELALU dikosongkan (&nbsp;) agar bisa ditulis tangan manual
  let noDisplay = '&nbsp;';
  if (!isTenant) {
    noDisplay = noInput || '001';
  }
    ...
    <div class="kwitansi-box-frame">
      ...
      <table class="kwitansi-body-table">
        <tr>
          ...
          <td class="kwitansi-val-col font-bold">${noDisplay}</td>
        </tr>
        <tr>
...
async function printSponsorshipKwitansi(saveHistory = true) {
  ...
  if (saveHistory) {
    ...
    const isTenant = (tipeJenis === 'STAND' || tipeJenis === 'TENANT');
    const noInput = document.getElementById('kwitansi-no')?.value?.trim() || '';
    const no = isTenant ? noInput : (noInput || '001');
    const tgl = document.getElementById('kwitansi-tgl')?.value || '';
    const dari = document.getElementById('kwitansi-dari')?.value || (isTenant ? 'Penyewa Tenant' : 'Sponsor / Donatur');
    ...
    renderSponsorshipHistoryTable();
    showToast('Kwitansi Disimpan', no ? `Kwitansi No. ${no} berhasil dicatat ke Riwayat.` : 'Kwitansi Tenant (Manual) berhasil dicatat ke Riwayat.', 'success');
...
function renderPengeluaranHistory() {
  const tbody = document.getElementById('pengeluaran-history-tbody');
  if (!tbody) return;

  const outTxns = (state.transactions || [])
    .filter(t => t.tipe === 'OUT')
    .sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal) || b.id.localeCompare(a.id));

  const totalOut = outTxns.reduce((sum, t) => sum + (parseInt(t.nominal, 10) || 0), 0);
  const totalEl = document.getElementById('pengeluaran-history-total');
  const countEl = document.getElementById('pengeluaran-history-count');
  if (totalEl) totalEl.textContent = formatRupiah(totalOut);
  if (countEl) countEl.textContent = `${outTxns.length} Transaksi`;

  if (outTxns.length === 0) {
    tbody.innerHTML = '<tr><td colspan="11" class="text-center text-muted" style="padding: 24px;">Belum ada transaksi pengeluaran yang tercatat.</td></tr>';
    return;
  }

  tbody.innerHTML = '';
  outTxns.forEach((tx, index) => {
    const tr = document.createElement('tr');
    tr.style.cursor = 'pointer';
    tr.addEventListener('click', () => showTransactionDetail(tx));
    ...
    tr.innerHTML = `
      ...
      <td>${tx.pic}</td>
      <td class="text-muted">${tx.keterangan}</td>
      <td class="text-right font-bold text-danger">${formatRupiah(tx.nominal)}</td>
      <td>${attHtml}</td>
      <td class="no-print text-center">${syncBadge}</td>
      <td class="no-print text-center" style="white-space: nowrap;">
        <div class="action-buttons-group">
          <button class="btn-table-action btn-action-edit btn-edit-pengeluaran" title="Edit Transaksi">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
          </button>
          <button class="btn-table-action btn-action-view btn-view-pengeluaran" title="Lihat Detail Transaksi">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
          </button>
        </div>
      </td>
    ...
    const editBtn = tr.querySelector('.btn-edit-pengeluaran');
    if (editBtn) {
      editBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        openEditModal(tx);
    ...
    const viewBtn = tr.querySelector('.btn-view-pengeluaran');
    if (viewBtn) {
      viewBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        showTransactionDetail(tx);
...
// ================= RENDER: RIWAYAT PENGAMBILAN UANG =================
function renderPengambilanHistory() {
  const tbody = document.getElementById('pengambilan-history-tbody');
  if (!tbody) return;

  const allOut = (state.transactions || [])
    .filter(t => t.tipe === 'OUT')
    .sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal) || b.id.localeCompare(a.id));

  const ambilTxns = allOut.filter(t => (
    (t.keterangan && t.keterangan.toUpperCase().includes('[PENGAMBILAN]')) ||
    (t.keterangan && t.keterangan.toUpperCase().includes('PENGAMBILAN'))
  ));

  // If specific [PENGAMBILAN] exists, show those, otherwise show all OUT transactions
  const displayList = ambilTxns.length > 0 ? ambilTxns : allOut;

  const totalAmbil = displayList.reduce((sum, t) => sum + (parseInt(t.nominal, 10) || 0), 0);
  const totalEl = document.getElementById('pengambilan-history-total');
  const countEl = document.getElementById('pengambilan-history-count');
  if (totalEl) totalEl.textContent = formatRupiah(totalAmbil);
  if (countEl) countEl.textContent = `${displayList.length} Pengambilan`;

  if (displayList.length === 0) {
    tbody.innerHTML = '<tr><td colspan="11" class="text-center text-muted" style="padding: 24px;">Belum ada catatan pengambilan uang.</td></tr>';
    return;
  }
  ...
  displayList.forEach((tx, index) => {
    ...
    tr.innerHTML = `
      ...
      <td class="no-print text-center" style="white-space: nowrap;">
        <div class="action-buttons-group">
          <button class="btn-table-action btn-action-edit btn-edit-pengambilan" title="Edit Transaksi">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
          </button>
          <button class="btn-table-action btn-action-view btn-view-pengambilan" title="Lihat Detail Transaksi">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
          </button>
        </div>
      </td>
    ...
    const editBtn = tr.querySelector('.btn-edit-pengambilan');
    if (editBtn) {
      editBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        openEditModal(tx);
    ...
    const viewBtn = tr.querySelector('.btn-view-pengambilan');
    if (viewBtn) {
      viewBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        showTransactionDetail(tx);
